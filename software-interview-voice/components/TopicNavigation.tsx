...
import { X, ChevronRight, ChevronDown, Server, Cloud, Layers, Database, Code, Users, Brain, Layout, GitMerge, Terminal } from './Icons';
...
const TopicNavigation: React.FC<TopicNavigationProps> = ({ language, onSelectTopic, isOpen, onClose }) => {
  ...
  const getIcon = (type: string) => {
    switch(type) {
      ...
      case 'algo': return <GitMerge className="w-4 h-4 text-purple-400" />;
      case 'system': return <Layout className="w-4 h-4 text-blue-400" />;
      case 'cloud': return <Cloud className="w-4 h-4 text-sky-400" />;
      case 'behavior': return <Users className="w-4 h-4 text-orange-400" />;
      ...
      >
        <div className="p-4 border-b border-stone-800 flex items-center justify-between bg-[#111]">
          <div className="flex items-center gap-2 text-stone-200 font-bold font-mono">
            <Brain className="w-5 h-5 text-tech-500" />
            <span>{language === 'zh' ? '面試準備索引' : 'Interview Index'}</span>
          </div>
