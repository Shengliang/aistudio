    ...
    <form onSubmit={handleSubmit} className="relative group z-10">
      ...
      <div className="relative flex items-center bg-[#0a0a0a] rounded-xl border border-stone-800 shadow-sm focus-within:border-tech-600 transition-all">
        ...
        <input
          ...
          placeholder={language === 'zh' ? "搜尋數據庫主題 (例如：'InnoDB Buffer Pool')..." : "Search database topic (e.g., 'InnoDB Buffer Pool')..."}
        ...
        <div className="pr-2 flex items-center gap-2">
          ...
          >
             {isSearching ? <Loader2 className="w-5 h-5 animate-spin" /> : <span className="font-bold text-sm px-2 font-mono">{language === 'zh' ? '執行' : 'EXEC'}</span>}
          </button>
        </div>
      </div>
      {isListening && (
